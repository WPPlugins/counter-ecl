...
/*
 ...
 Description: Web counter widget.
...
// old cookie
if ( isset( $_COOKIE[ 'ecl_cont_visited_cookie' ] ) ) {
	
	$ecl_cookie = maybe_unserialize( $_COOKIE['ecl_cont_visited_cookie'] );

	if ( isset( $ecl_cookie[ 'visited' ] ) 
		&& $ecl_cookie[ 'visited' ] == 'true'
		&& isset( $ecl_cookie[ 'domain' ] ) 
		&& $ecl_cookie[ 'domain' ] == $ecl_domain ) {
		
		
			$ecl_cont_visited = true;
			
			// clean old cookie
			setcookie( 'ecl_cont_visited_cookie', 
				'', 
				time() - 1, 
				'/', 
				$ecl_domain == 'localhost' ? null : $ecl_domain,
				is_ssl());
			
			// generate new cookie
			setcookie( 'ecl_cont_visited_cookie' . md5($ecl_domain),
				maybe_serialize( array('visited' => 'true', 'domain' => $ecl_domain )),
				time() + (ecl_cont_get_time_cookie_expire(false) * 24 * 60 * 60) ,
				'/',
				$ecl_domain == 'localhost' ? null : $ecl_domain,
				is_ssl());
	}
 ...
 elseif ( isset( $_COOKIE[ 'ecl_cont_visited_cookie' . md5($ecl_domain) ] ) ) {
	
	$ecl_cookie = maybe_unserialize( $_COOKIE['ecl_cont_visited_cookie'. md5($ecl_domain)] );
...
function ecl_cont_add_visitor( $arg='' ) {	
 	global $ecl_cont_visited;
 	if ( !$ecl_cont_visited && !is_admin() && !is_preview() ) {
...
add_filter( 'template_include', 'ecl_cont_add_visitor' );
 ...
 */
 ...
 function ecl_cont_register_init() {
 	...
 	wp_register_script('ecl-cont-js', plugins_url( 'js/counter-ecl.js', __FILE__ ) , array('jquery'), '1', true);
